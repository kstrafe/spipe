...
@defform/subs[(spipe hash transformation ...)
  ...
  (Note: If anyone can tell me how to make the r: in r:accessor non-italicized then please tell me (kefin on IRC: freenode, #racket), or by email macocio@"@"gmail.com)

  @itemize[

           @item{@racket[r:accessor] as well as their @racket[w:], @racket[rw:], and @racket[e:], counterparts are intended to be prepended to the identifier in order to match.
              @examples[#:eval evaluator
                (spipe (hash) (identity r:test-read w:test-write))
              ]
              @racket[spipe] looks for these prefixes to determine how to order arguments and store values inside the hash-table. @racket[x:] is never prepended as it expects an expression.
           }
